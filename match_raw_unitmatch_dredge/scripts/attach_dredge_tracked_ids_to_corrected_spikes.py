...
def main() -> None:
    ...
    corrected_root = Path(config["dredge_per_session_root"])
    attached_root = Path(config["corrected_spikes_root"])
    attached_root.mkdir(parents=True, exist_ok=True)

    all_summaries = []
    merged_output = attached_root / "al032_tracked_spikes_all_sessions_dredge.csv.gz"
    first_merged = True
    with gzip.open(merged_output, "wt") as merged_handle:
        for session_name in config["session_names"]:
            session_mapping = mapping.loc[
                mapping["session_name"] == session_name,
                ["cluster_id", "tracked_unit_id", "conflict_flag"],
            ].copy()
            corrected_path = corrected_root / session_name / f"{session_name}_corrected_spike_table.csv.gz"
            output_path = attached_root / session_name / f"{session_name}_tracked_spikes.csv.gz"
            summary = write_chunked_join(corrected_path, session_mapping, session_name, output_path)
            all_summaries.append(summary)
            for chunk in pd.read_csv(output_path, compression="gzip", chunksize=250_000, low_memory=False):
                chunk.to_csv(merged_handle, index=False, header=first_merged)
                first_merged = False

    dump_json(
        attached_root / "attach_dredge_tracked_ids_summary.json",
        {
            "created_at": now_iso(),
