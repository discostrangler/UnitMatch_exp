...
def main() -> None:
    ...
    os.environ.setdefault(
        "MPLCONFIGDIR",
        "/scratch/am15577/UnitMatch/match_raw_unitmatch_dredge/tmp/matplotlib",
    )

    match_table_path = save_dir / "MatchTable.csv"
    if match_table_path.exists() and not args.force:
        print(f"DREDge UnitMatch output already exists at {save_dir}; skipping. Use --force to rerun.")
        return
    ...
    print(f"Loaded {param['n_units']} good units across {param['n_sessions']} sessions for DREDge UnitMatch")
    extracted_wave_properties = ov.extract_parameters(waveform, channel_pos, clus_info, param)

    shift_df = pd.read_csv(config["geometry_shift_by_unit_csv"])
    shift_df = shift_df.sort_values("unitmatch_unit_index")
    if int(shift_df.shape[0]) != int(param["n_units"]):
        raise RuntimeError(
            f"Geometry shift table row count {shift_df.shape[0]} does not match UnitMatch n_units {param['n_units']}"
        )
    shifts = shift_df["median_y_shift_um"].fillna(0.0).to_numpy(dtype=np.float64)

    extracted_wave_properties["avg_centroid"] = extracted_wave_properties["avg_centroid"].copy()
    extracted_wave_properties["avg_waveform_per_tp"] = extracted_wave_properties["avg_waveform_per_tp"].copy()
    extracted_wave_properties["avg_centroid"][1, :, :] += shifts[:, None]
    extracted_wave_properties["avg_waveform_per_tp"][1, :, :, :] += shifts[:, None]
    ...
    run_summary = {
        ...
        "median_abs_applied_shift_um": float(np.median(np.abs(shifts))),
        "mean_abs_applied_shift_um": float(np.mean(np.abs(shifts))),
    }
